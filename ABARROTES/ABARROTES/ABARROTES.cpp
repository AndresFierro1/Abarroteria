...
struct Deudor {
    string nombre;
    vector<string> productos; // Almacena productos en formato "nombre,precio,fecha"
    int monto;
    string fecha;
};
...
string dateToString(const chrono::system_clock::time_point& timePoint) {
    std::time_t time_t_value = chrono::system_clock::to_time_t(timePoint);
...
void operaciones(int r) {
    if (r == 4) {
        cout << setw(10) << "Nombre:" << setw(12) << "Producto:" << setw(5) << " Precio: " << setw(14) << " Fecha: " << endl;
        for (int i = 0; i < listad.size(); i++) {
            cout << setw(10) << deudores[i].nombre << setw(12) << deudores[i].productos[0] << setw(5) << deudores[i].monto << setw(14) << deudores[i].fecha << endl;
        }
    }
    else if (r == 3 || r == 5) {
        realizarCompra(r == 3);
        ...
        auto it = find_if(deudores.begin(), deudores.end(), [&](Deudor& d) { return d.nombre == nombre; });
        if (it != deudores.end()) {
            cout << "Deudor encontrado: " << it->nombre << ", Monto: $" << it->monto << ", Fecha: " << it->fecha << endl;
        }
        else {
            cout << "Deudor no encontrado." << endl;
        }
        ...
        guardarStock(); // Guardar stock antes de salir
...
void guardarDeudores() {
    ...
    for (const auto& deudor : deudores) {
        for (const auto& producto : deudor.productos) {
            archivoDeudoresOut << deudor.nombre << "," << producto << endl;
        }
    }
...
void realizarCompra(bool esCredito) {
    ...
    int totalMonto = 0;
    string fech = dateToString(date_now);
    int opcion;
        ...
        if (opcion > 0 && opcion <= productos.size()) {
            if (productos[opcion - 1].stock > 0) {
                ...
                if (cantidad > 0 && cantidad <= productos[opcion - 1].stock) {
                    ...
                    if (esCredito) {
                        string productoInfo = productos[opcion - 1].name + "," + to_string(productos[opcion - 1].price) + "," + fech;
                        deudores.push_back({ nom, {productoInfo}, totalMonto, fech });
                    }
                }
                else {
...
void gestionarProveedor() {
    int opcion;
    cout << "Elige un producto para actualizar el stock: ";
    cin >> opcion;
...
int main() {
    ...
    while (getline(archivoIn, line)) {
        ...
        try {
            ...
            deudores.push_back({ nomb, productosComprados, monto, fc });
        }
        catch (const invalid_argument& e) {
